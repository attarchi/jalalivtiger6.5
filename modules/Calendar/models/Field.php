...
class Calendar_Field_Model extends Vtiger_Field_Model {
	...
	*/
	...
	public function getEditViewDisplayValue($value) {
		...
		//changed by favtiger.ir
		//Set the start date and end date
		if(empty($value)) {
			if ($fieldName === 'date_start') {
				return DateTimeField::convertToUserFormat(DateTimeField::gregorian_to_jalali(date('Y-m-d')));
			} elseif ($fieldName === 'due_date') {
				$currentUser = Users_Record_Model::getCurrentUserModel();
				$minutes = $currentUser->get('callduration');
				return DateTimeField::convertToUserFormat(DateTimeField::gregorian_to_jalali(date('Y-m-d', strtotime("+$minutes minutes"))));
			}
		}else{
			if ($fieldName === 'date_start') {
				$value = DateTimeField::gregorian_to_jalali($value);
			} elseif ($fieldName === 'due_date') {
				$value = DateTimeField::gregorian_to_jalali($value);
			}
		}
