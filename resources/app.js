...
var app = {
	...
	registerEventForDatePickerFields : function(parentElement,registerForAddon,customParams){
		...
		element.each(function(index,domElement){
			...
			if(customParams) {
				jQelement.multiDatesPicker({
					maxPicks: 2,
					dateFormat: vtigerDateFormat.replace(/Y/i, "yy"),
				...
				jQelement.datepicker({
					...
					onSelect: function(dateStr) {
						var getcurrent_datepickerid = jQuery(this).attr('id');
						jQuery("input[id='" + getcurrent_datepickerid + "']").val(dateStr);
					}
				});
			}
		});
