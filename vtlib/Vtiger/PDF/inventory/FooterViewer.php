...
class Vtiger_PDF_InventoryFooterViewer extends Vtiger_PDF_FooterViewer {
	...
	function display($parent) {
		...
		if($this->model) {
			...
			$pdf->MultiCell($footerFrame->w, $descriptionHeight, $descriptionString, 1, 'R', 1, 1, $footerFrame->x, $footerFrame->y);
			$pdf->MultiCell($footerFrame->w, $targetFooterHeight - $descriptionHeight, $description, 1, 'R',
			...
			$pdf->SetFillColor(205,201,201);
			$pdf->MultiCell($footerFrame->w, $termsAndConditionHeight, $termsAndConditionLabelString, 1, 'R', 1, 1,
				$pdf->GetX(), $pdf->GetY() + $offsetY);
			$pdf->MultiCell($footerFrame->w, $targetFooterHeight - $termsAndConditionHeight, $termsAndCondition,1, 'R', 0, 1,
				$pdf->GetX(), $pdf->GetY());
		}
	}
}
