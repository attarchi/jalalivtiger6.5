...
class Vtiger_PDF_InventoryHeaderViewer extends Vtiger_PDF_HeaderViewer {
	...
	function display($parent) {
		...
		if($this->model) {
			...
			$pdf->MultiCell($headerColumnWidth, $contentHeight+5, $modelColumn0['summary'], 0, 'R', 0, 1, 
				$headerFrame->x, $headerFrame->y+$imageHeightInMM+10);
			
			$pdf->SetFont($fontname, '');
			$contentHeight = $pdf->GetStringHeight( $modelColumn0['content'], $headerColumnWidth);			
			$pdf->MultiCell($headerColumnWidth, $contentHeight, $modelColumn0['content'], 0, 'R', 0, 1, 
			...
			$pdf->SetFont($fontname, 'B');
			$pdf->MultiCell($contentWidth*2.0, $contentHeight, $this->model->get('title'), 0, 'L', 0, 1, $contentX-$contentWidth,
				 $headerFrame->y+2);
			...
			foreach($modelColumn2 as $label => $value) {
				if(is_array($value)) {
					...
					foreach($value as $l => $v) {
						...
						if($temp[2]>1600){
							$temp = pgregorian_to_jalali($temp[2],$temp[1],$temp[0]);
				...
				$pdf->SetFillColor(205,201,201);
                                if($label=='Shipping Address'){ 
                                    ...
                                    $pdf->MultiCell($headerColumnWidth-$offsetX, 7, $label, 1, 'C', 1, 1, $headerFrame->x+$headerColumnWidth*2.0+$offsetX,
                                            $pdf->GetY()+$offsetY-$height-$offsetX-4.0); 

                                    $pdf->SetFont($fontname, '');
                                    $pdf->MultiCell($headerColumnWidth-$offsetX, 7, $value, 1, 'C', 0, 1, $headerFrame->x+$headerColumnWidth*2.0+$offsetX, 
									$pdf->GetY());
								} else{ 
                                    $pdf->MultiCell($headerColumnWidth-$offsetX, 7, $label, 1, 'C', 1, 1, $headerFrame->x+$headerColumnWidth*2+2, 
                                            $pdf->GetY()+$offsetY); 

                                    $pdf->SetFont($fontname, ''); 
                                    $pdf->MultiCell($headerColumnWidth-$offsetX, 7, $value, 1, 'C', 0, 1, $headerFrame->x+$headerColumnWidth*2+2,  
                                            $pdf->GetY()); 
                                    } 
                                } 
                            } 
