...
require_once 'include/pdate.php';
class DateTimeField {
	...
	function getDBInsertDateValue($user = null) {
		...
		$log->debug("Exiting getDBInsertDateValue method ...".$insert_date);
		$insert_date = self::jalali_to_gregorian($insert_date);
		return $insert_date;
	}
	...
	public function getDisplayDateTimeValue ($user = null) {
		//echo $this->getDisplayDate($user) . ' ' . $this->getDisplayTime($user);
		return $this->getDisplayDate($user) . ' ' . $this->getDisplayTime($user);
	}

    public function getFullcalenderDateTimevalue ($user = null) {
		return $this->getDisplayDateG($user) . ' ' . $this->getFullcalenderTime($user);
	}
	/**
	 *
	...
	function getDBInsertTimeValue($user = null) {
		...
		$value = explode(' ', $this->datetime);
		$insert_date = self::convertToDBFormat($value[0]);
		$temp = explode('-', $insert_date);
		$value[0] = $temp[0].'-'.$temp[1].'-'.$temp[2];
		$gdate = self::jalali_to_gregorian($value[0]);
		$gdate = $gdate.' '.$value[1];
		$log->debug("vahid Entering getDBInsertTimeValue(" . $gdate . ") method ...");
		$date = self::convertToDBTimeZone($gdate, $user);
	 ...
	 * @return string
	 */
	function getDisplayDateG( $user = null ) {
		global $log;
		...
		return self::gregorian_to_jalali($display_date);
	}
	/**
	 * This function returns the date in user specified format.
	 * @global type $log
	...
	function getDisplayDate( $user = null ) {
		...
		if(is_array($date_value))
			$date_value = self::gregorian_to_jalali($date_value[0]);
		else{
			$date_value = self::gregorian_to_jalali($date_value);
		}
		$display_date = self::convertToUserFormat($date_value, $user);
		$log->debug("Exiting getDisplayDate method ...");
		//echo $display_date .'this<br />';
		return $display_date;
	}
	...
	public static function gregorian_to_jalali($date){
		$date_value = $date;
		$temp =explode('-',$date_value);
		if($temp[0]>1600){
			$temp = pgregorian_to_jalali($temp[0],$temp[1],$temp[2]);
			$date_value = $temp[0].'-'.$temp[1].'-'.$temp[2];
		}
		return $date_value;
	}
	...
	public static function jalali_to_gregorian($date){
		...
		$temp = explode('-',$insert_date);
		if($temp[0]<1600){
			$temp = pjalali_to_gregorian($temp[0],$temp[1],$temp[2]);
			$insert_date = $temp[0].'-'.$temp[1].'-'.$temp[2];
		}
		return $insert_date;
	}
