...
class Vtiger_PDF_InventoryContentViewer extends Vtiger_PDF_ContentViewer {
	...
	function initDisplay($parent) {
		...
		$pdf->MultiCell($contentFrame->w, $contentFrame->h, "", 1, 'R', 0, 1, $contentFrame->x, $contentFrame->y);
		...
		foreach($this->cells as $cellName => $cellWidth) {
			...
			$pdf->MultiCell($cellWidth, $this->headerRowHeight, $cellLabel, 1, 'R', 0, 1, $contentFrame->x+$offsetX, $contentFrame->y);
			$offsetX += $cellWidth;
		}
		$pdf->SetFont('','');
		// Reset the y to use
	...
	function drawCellBorder($parent, $cellHeights=False) {		
		...
		foreach($this->cells as $cellName => $cellWidth) {
			...
			$pdf->MultiCell($cellWidth, $cellHeight, "", 1, 'R', 0, 1, $contentFrame->x+$offsetX, $offsetY);
			$offsetX += $cellWidth;
		}
	}
	...
	function displayPreLastPage($parent) {
		...
		for ($index = 0; $index < $totalModels; ++$index) {
			...
			if (!empty($commentContent)) {
				...
				$pdf->MultiCell($commentCellWidth, $contentHeight, $model->get('Comment'), 0, 'R', 0, 1, $contentLineX+$offsetX,
					 $contentLineY);
		...
		if ($this->contentSummaryModel) {
			...
			foreach($summaryCellKeys as $key) {	
				$pdf->MultiCell($summaryCellLabelWidth, $summaryCellHeight, $key, 1, 'R', 0, 1, $summaryLineX, $summaryLineY);
