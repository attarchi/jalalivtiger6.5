...
function pdate($format, $timestamp = NULL)
{
    ...
    list($pYear, $pMonth, $pDay) = pgregorian_to_jalali($gYear, $gMonth, $gDay);
    $pWeek = ($gWeek + 1);
...
function pmktime($hour = 0, $minute = 0, $second = 0, $month = 0, $day = 0, $year = 0, $is_dst = -1)
{
    ...
    list($year, $month, $day) = pjalali_to_gregorian($year, $month, $day);
...
function pdiv($a=1, $b=1)
{
	return (int)($a / $b);
}

function pgregorian_to_jalali ($g_y, $g_m, $g_d)
{
    ...
    $gm = ($g_m - 1);
    $gd = ($g_d - 1);
    $g_day_no = (365 * $gy + pdiv($gy + 3, 4) - pdiv($gy + 99, 100) + pdiv($gy + 399, 400));
    ...
    $j_np = pdiv($j_day_no, 12053); # 12053 = (365 * 33 + 32 / 4)
    $j_day_no = ($j_day_no % 12053);
    $jy = (979 + 33 * $j_np + 4 * pdiv($j_day_no, 1461)); # 1461 = (365 * 4 + 4 / 4)
    $j_day_no %= 1461;

        if($j_day_no >= 366)
        { 
            $jy += pdiv($j_day_no - 1, 365);
...
function pjalali_to_gregorian($j_y, $j_m, $j_d)
{
    static $g_days_in_month = array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
    static $j_days_in_month = array(31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29);
    $jy = ($j_y - 979);
    $jm = ($j_m - 1);
    $jd = ($j_d - 1);
    $j_day_no = (365 * $jy + pdiv($jy, 33) * 8 + pdiv($jy % 33 + 3, 4));
    ...
    $gy = (1600 + 400 * pdiv($g_day_no, 146097)); # 146097 = (365 * 400 + 400 / 4 - 400 / 100 + 400 / 400)
    ...
    $leap = 1;
        ...
        {
            $g_day_no --;
            $gy += (100 * pdiv($g_day_no, 36524)); # 36524 = (365 * 100 + 100 / 4 - 100 / 100)
    ...
    $gy += (4 * pdiv($g_day_no, 1461)); # 1461 = (365 * 4 + 4 / 4)
    $g_day_no %= 1461;
